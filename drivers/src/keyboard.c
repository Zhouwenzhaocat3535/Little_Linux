...
volatile int kbuf_head;
volatile int kbuf_tail;
volatile char keyboard_buffer[KBUF_SIZE];
...
static inline void putchar_to_buffer(char c) {
    int next = (kbuf_head + 1) % KBUF_SIZE;
    if (next != kbuf_tail) {  // 缓冲区未满
        keyboard_buffer[kbuf_head] = c;
        kbuf_head = next;
    }
}
...
char getchar_from_buffer(void) {
    while (kbuf_head == kbuf_tail) {
		// 缓冲区为空，等待中断
        __asm__("hlt");  // 暂停 CPU，等待下一个中断
    }
    char c = keyboard_buffer[kbuf_tail];
    kbuf_tail = (kbuf_tail + 1) % KBUF_SIZE;
    return c;
}

char getchar_from_buffer_timeout(unsigned int timeout_ms) {
    // 假设 CPU 主频 1GHz，1ms ≈ 1,000,000 个空循环（粗略估计）
    unsigned long loops_per_ms = 1000000;
    unsigned long max_loops = timeout_ms * loops_per_ms;
    
    for (unsigned long i = 0; i < max_loops; i++) {
        ...
        __asm__("pause");  // 提示 CPU 这是自旋等待，降低功耗
    }
